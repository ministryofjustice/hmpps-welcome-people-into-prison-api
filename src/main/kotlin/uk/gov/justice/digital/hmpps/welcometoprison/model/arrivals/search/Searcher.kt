...
class Searcher<Q, T : Comparable<T>>(private val strategy: SearchStrategy<Q, T>) {
  ...
  fun search(query: Q?, input: List<T>): List<T> = searchWithRelevance(query, input).map { it.item }
  ...
  private fun List<T>.findRelevantResults(query: Q) = this.map { strategy.evaluate(query, it) }.filter { it.relevance != null }
