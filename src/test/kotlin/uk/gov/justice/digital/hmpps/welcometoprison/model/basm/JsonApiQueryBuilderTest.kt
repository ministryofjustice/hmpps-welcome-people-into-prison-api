...
class JsonApiQueryBuilderTest {
  ...
  @Test
  fun `empty query`() = queryOf() `decodes to` ""

  @Test
  fun `includes only`() = queryOf(includes = listOf("profile.person", "profile.person_escort_record.flags")) `decodes to`
    "?include=profile.person,profile.person_escort_record.flags"

  @Test
  fun `single filter`() = queryOf(filters = mapOf("name" to listOf("Jim"))) `decodes to` "?filter[name]=Jim"

  @Test
  fun `multiple filters`() = queryOf(
    filters = mapOf(
      "name" to listOf("Jim"),
      "age" to listOf("65"),
    ),
  ...
  @Test
  fun `includes and filters`() = queryOf(
...
infix fun String.`decodes to`(expected: String) = Assertions.assertEquals(expected, URLDecoder.decode(this, UTF_8))
